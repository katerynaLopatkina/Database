...
def search_teachers(cur, first_name, subject_name, max_credits, min_credits):
    sql = "SELECT teacher.id, first_name, last_name, phone_number, " \
          "s.name as subject_name, s.time as credits, s.id as id_subject " \
          "from teacher " \
...
def search_students_by_teacher_and_group(cur, first_name, group_name, start_date, end_date):
    sql = "SELECT DISTINCT s.id as id_student, s.first_name as first_name_s, s.last_name as last_name_s," \
          " s.date_of_birth, gr.name as group_name, " \
          "gr.id as id_group, sb.id as id_subject, sb.name as sb_name, teach.id as id_teacher, " \
          "teach.first_name as first_name_t, " \
