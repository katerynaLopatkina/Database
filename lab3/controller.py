...
class Controller(object):
    ...
    def show_option(self, table_name):
        if table_name == TEACHER_SUBJECT_TABLE:
            id_t = self.get_id("teacher")
            id_s = self.get_id("subject")
            self.show_one_by_ids(id_s, id_t, table_name)
        elif table_name == STUDENT_SUBJECT_TABLE:
            id_s = self.get_id("student")
            id_sb = self.get_id("subject")
            self.show_one_by_ids(id_sb, id_s, table_name)
        elif table_name == SUBJECT_GROUP_TABLE:
            id_g = self.get_id("group")
            id_sb = self.get_id("subject")
            self.show_one_by_ids(id_sb, id_g, table_name)
        else:
    ...
    def show_one_by_ids(self, id1, id2, table_name):
        grade = self.model.get_one_by_ids(id1, id2, table_name)[0]
        self.view.show_item(grade, 0, table_name)
    ...
    def add_student_option(self):
        ...
        data_to_add = [None, first_name, last_name, sex, dat_of_birth, id_group]
    ...
    def add_teacher_option(self):
        ...
        data_to_add = [None, first_name, last_name, phone]
    ...
    def add_teacher_subject_option(self):
        ...
        data_to_add = [None, id_teacher, id_subject]
    ...
    def add_student_subject_option(self):
        ...
        data_to_add = [None, id_student, id_subject]
    ...
    def add_subject_group_option(self):
        ...
        data_to_add = [None, id_subject, id_group]
    ...
    def delete_option(self, table):
        ...
        if table == SUBJECT_GROUP_TABLE:
            id_group = self.get_id("group")
            id_subject = self.get_id("subject")
            self.delete_by_ids(id_subject, id_group, table)
        elif table == TEACHER_SUBJECT_TABLE:
            id_teacher = self.get_id("teacher")
            id_subject = self.get_id("subject")
            self.delete_by_ids(id_subject, id_teacher, table)
        elif table == STUDENT_SUBJECT_TABLE:
            id_student = self.get_id("student")
            id_subject = self.get_id("subject")
            self.delete_by_ids(id_subject, id_student, table)
        else:
    ...
    def delete_by_ids(self, id1, id2, table):
        try:
            item = self.model.delete_by_ids(id1, id2, table)
            self.view.display_deletion_result(item, table)
        except Exception as e:
            self.view.display_exception(table, e)
    ...
    def update_student_subject_option(self):
        ...
        subject = self.model.get_one_by_ids(id_subject, id_student, STUDENT_SUBJECT_TABLE)[0]
        if subject is not None:
            ...
            while menu:
                ...
                if option.isdigit():
                    if int(option) == 1:
                        id_s = self.get_id("subject")
                        new_subject = self.model.get_one(id_s, SUBJECT_TABLE)
                        ...
                        id_st = self.get_id('student')
                        new_student = self.model.get_one(id_st, SUBJECT_TABLE)
                    ...
                    self.view.show_number_exception(option)
            try:
                self.update([id_student, id_subject, student_id, subject_id], STUDENT_SUBJECT_TABLE)
            except Exception as e:
    ...
    def update_teacher_subject_option(self):
        ...
        teacher = self.model.get_one_by_ids(id_subject, id_teacher, TEACHER_SUBJECT_TABLE)[0]
        if teacher is not None:
            self.view.show_item(teacher, 0, TEACHER_SUBJECT_TABLE)
            while menu:
                ...
                if option.isdigit():
                    if int(option) == 1:
                        ...
                        new_teacher = self.model.get_one(id_t, TEACHER_TABLE)
                    ...
                    self.view.show_number_exception(option)
            try:
                self.update([id_teacher, id_subject, teacher_id, subject_id], TEACHER_SUBJECT_TABLE)
            except Exception as e:
    ...
    def update_subject_group_option(self):
        ...
        subject_id = id_subject
        group_id = id_group
        subject = self.model.get_one_by_ids(id_subject, id_group, SUBJECT_GROUP_TABLE)[0]
        if subject is not None:
            ...
            while menu:
                ...
                if option.isdigit():
                    if int(option) == 1:
                        id_s = self.get_id("subject")
                        print("Befor subkect fetch")
                        new_subject = self.model.get_one(id_s, SUBJECT_TABLE)
                        print("After subkect fetch")
                        if new_subject is None:
                            self.view.display_item_not_yet_stored_error(SUBJECT_GROUP_TABLE, SUBJECT_GROUP_TABLE, None)
                        else:
                            subject_id = new_subject.id
                        ...
                        id_t = self.get_id('Group id')

                        print("Befor group fetch")
                        new_group = self.model.get_one(id_t, GROUP_TABLE)
                        print("After group fetch")
                        if new_group is None:
                            self.view.display_item_not_yet_stored_error(SUBJECT_GROUP_TABLE, SUBJECT_GROUP_TABLE, None)
                        else:
                            print("Befor group _id assign")
                            group_id = new_group.id
                            print("After group _id assign")
                    elif int(option) == 0:
                        menu = False
                    else:
                        self.view.no_such_menu()
            ...
            try:
                ...
                self.update([id_group, id_subject, group_id, subject_id], SUBJECT_GROUP_TABLE)
            except Exception as e:
        ...
        try:
            ...
            if res is None:
                ...
                if table_name == SUBJECT_GROUP_TABLE \
                        or table_name == STUDENT_SUBJECT_TABLE \
                        or table_name == TEACHER_SUBJECT_TABLE:
                    table_name = table_name+"old"
    ...
    def search_students_by_teacher_and_group(self):
        ...
        group_name = self.view.get_field(STUDENT_TABLE, 'Group name')
        group_name = self.check_values(group_name, True, STUDENT_TABLE, 'Group name')
    ...
    def check_values(self, value, condition, name, field):
        ...
        while len(val) == 0 or not condition:
            self.view.display_value_exception()
